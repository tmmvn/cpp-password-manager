...
void TestKeys::testComposite()
{
	...
	PasswordKey* passwordKey2 = new PasswordKey(
		"test"
	);
	bool ok;
	QString errorString;
	// make sure that addKey() creates a copy of the keys
	compositeKey1->addKey(
		*passwordKey1
	);
	compositeKey1->addKey(
		*passwordKey2
	...
	delete passwordKey1;
	delete passwordKey2;
	QByteArray transformed = compositeKey1->transform(
		QByteArray(
			32,
			'\0'
		),
	...
	QCOMPARE(
		compositeKey2->transform(QByteArray(32, '\0'), 1, &ok, &errorString),
		transformed
	);
	QVERIFY(
		ok
	...
	compositeKey3->clear();
	QCOMPARE(
		compositeKey3->rawKey(),
		compositeKey4->rawKey()
	);
...
void TestKeys::testCreateFileKey()
{
	...
	delete dbOrg;
	KeePass2Reader reader;
	Database* dbRead = reader.readDatabase(
		&dbBuffer,
		compositeKey
	);
	QVERIFY(
...
{
	QByteArray env = qgetenv(
		"BENCHMARK"
	);
	if(env.isEmpty() || env == "0" || env == "no")
	{
		QSKIP(
			"Benchmark skipped. Set env variable BENCHMARK=1 to enable."
		);
	}
	PasswordKey pwKey;
	pwKey.setPassword(

...
void TestKeePass2RandomStream::test()
{
	...
	QVERIFY(
		cipher.init(CryptoHash::hash(key, CryptoHash::Sha256), KeePass2::
	...
	bool ok;
	QVERIFY(
		randomStream.init(key)
	);
	QByteArray randomStreamData;
	randomStreamData.append(
		randomStream.process(
			data.mid(
				0,
				7
			),
	...
	QVERIFY(
		cipherEncrypt.init(CryptoHash::hash(key, CryptoHash::Sha256), KeePass2::
			INNER_STREAM_SALSA20_IV)
	);
	QByteArray cipherDataEncrypt = cipherEncrypt.process(
		data,
		&ok
	);
	QVERIFY(

...
void TestEntrySearcher::testSearch()
{
	...
	group1->setSearchingEnabled(
		Group::Disable
	);
	group11->setSearchingEnabled(
		Group::Enable
	);
	Entry* eRoot = new Entry();
	eRoot->setNotes(
		"test search term test"
	);
	eRoot->setGroup(
		m_groupRoot
	);
	...
	m_searchResult = m_entrySearcher.search(
		"search term",
		m_groupRoot,
		Qt::CaseInsensitive
	);
...
void TestEntrySearcher::testAllAttributesAreSearched()
{
	...
	m_searchResult = m_entrySearcher.search(
		"testTitle testUsername testUrl testNote",
		m_groupRoot,
		Qt::CaseInsensitive
	);
	QCOMPARE(

...
void ModelTest::nonDestructiveBasicTest()
{
	...
	fetchingMore = false;
	Qt::ItemFlags flags = model->flags(
		QModelIndex()
	);
	QVERIFY(
		flags == Qt::ItemIsDropEnabled || flags == 0
	);
	model->hasChildren(
		QModelIndex()
	);
...
void ModelTest::rowCount()
{
	//     qDebug() << "rc";
	// check top row
	QModelIndex topIndex = model->index(
		0,
		0,
		QModelIndex()
	);
	...
	QModelIndex secondLevelIndex = model->index(
		0,
		0,
		topIndex
	);
...
void ModelTest::columnCount()
{
	...
	QModelIndex childIndex = model->index(
		0,
		0,
		topIndex
	);
...
void ModelTest::index()
{
	...
	QModelIndex a = model->index(
		0,
		0
	);
	QModelIndex b = model->index(
		0,
		0
	);
	QVERIFY(
...
void ModelTest::parent()
{
	...
	QModelIndex topIndex1 = model->index(
		0,
		1,
		QModelIndex()
	);
	...
	) > 0)
	{
		...
		QModelIndex childIndex1 = model->index(
			0,
			0,
			topIndex1
		);
...
)
{
	...
	const QModelIndex topLeftChild = model->index(
		0,
		0,
		parent
	);
	QVERIFY(
		!model->hasIndex ( rows + 1, 0, parent )
	);
	for(int r = 0; r < rows; ++r)
	{
		...
		QVERIFY(
			!model->hasIndex ( r, columns + 1, parent )
		);
		for(int c = 0; c < columns; ++c)
		{
			QVERIFY(
				model->hasIndex ( r, c, parent )
			);
			...
			QModelIndex modifiedIndex = model->index(
				r,
				c,
				parent
			);
			...
			QVERIFY(
				...
				const QModelIndex sibling = model->sibling(
					r,
					c,
					topLeftChild
				);
				...
				const QModelIndex sibling = topLeftChild.sibling(
					r,
					c
				);
				QVERIFY(
			...
			{
				qDebug() << r << c << currentDepth << model->data(
					index
				).toString() << model->data(
					parent
				).toString();
				qDebug() << index << parent << model->parent(
					index
				);
			...
			QModelIndex newerIndex = model->index(
				r,
				c,
				parent
			);
...
void ModelTest::data()
{
	...
	QVERIFY(
		!model->setData ( QModelIndex(), QLatin1String ( "foo" ), Qt::
			DisplayRole )
	);
	// General Purpose roles that should return a QString
	QVariant variant = model->data(
		model->index(
			0,
			0
		),
	...
	{
		QVERIFY(
			variant.canConvert<QSize>()
		);
	}
	// General Purpose roles that should return a QFont
	QVariant fontVariant = model->data(
		model->index(
			0,
			0
		),
	...
	{
		QVERIFY(
			fontVariant.canConvert<QFont>()
		);
	}
	// Check that the alignment is one we know about
	QVariant textAlignmentVariant = model->data(
		model->index(
			0,
			0
		),
	...
	if(textAlignmentVariant.isValid())
	{
		...
		QCOMPARE(
			alignment,
			( alignment & static_cast<int>( Qt::AlignHorizontal_Mask | Qt::
				AlignVertical_Mask ) )
		);
	}
	// General Purpose roles that should return a QColor
	QVariant colorVariant = model->data(
		model->index(
			0,
			0
		),
	...
	{
		QVERIFY(
			colorVariant.canConvert<QColor>()
		);
	}
	colorVariant = model->data(
		model->index(
			0,
			0
		),
	...
	QVariant checkStateVariant = model->data(
		model->index(
			0,
			0
		),
	...
	if(checkStateVariant.isValid())
	{
		...
		QVERIFY(
			state == Qt::Unchecked || state == Qt::PartiallyChecked || state ==
			Qt::Checked
		);
	}
}
...
)
{
	...
	//    qDebug();
	QVERIFY(
		c.oldSize + ( end - start + 1 ) == model->rowCount ( parent )
	);
	QVERIFY(
		c.last == model->data ( model->index ( start - 1, 0, c.parent ) )
	);
	if(c.next != model->data(
		model->index(
			end + 1,
			0,
			c.parent
	...
	QVERIFY(
		c.next == model->data ( model->index ( end + 1, 0, c.parent ) )
	);
}

void ModelTest::layoutAboutToBeChanged()
{
	for(int i = 0; i < qBound(
			0,
			model->rowCount(),
			100
		); ++i)
...
void ModelTest::layoutChanged()
{
	for(int i = 0; i < changing.count(); ++i)
	{
		...
		QVERIFY(
			p == model->index ( p.row(), p.column(), p.parent() )
		);
	}
	changing.clear();
}
...
)
{
	...
	QVERIFY(
		c.next == model->data ( model->index ( start, 0, c.parent ) )
	);
}
...
)
{
	...
	QVERIFY(
		bottomRight.row() < rowCount
	);
	QVERIFY(
		bottomRight.column() < columnCount
	);
}

void ModelTest::headerDataChanged(
...
)
{
	...
	int itemCount = orientation == Qt::Vertical
		? model->rowCount()
		: model->columnCount();
	QVERIFY(
		start < itemCount
...
)
{
	...
	{
		QVERIFY(
			cd.oldSize + ( end - start + 1 ) == model->rowCount ( destParent )
		);
		QVERIFY(
			cd.last == model->data ( model->index ( destinationRow - 1, 0, cd.
				parent ) )
		);
		QVERIFY(
			cd.next == model->data ( model->index ( destinationRow + (end -
				start + 1), 0, cd.parent ) )
		);
	}
	...
	{
		QVERIFY(
			cs.oldSize - ( end - start + 1 ) == model->rowCount ( srcParent )
		);
		QVERIFY(
			cs.last == model->data ( model->index ( start - 1, 0, srcParent ) )
		);
		QVERIFY(
			cs.next == model->data ( model->index ( start, 0, srcParent ) )
		);
	}
}

...
const QString TestCsvExporter::ExpectedHeaderLine = QString(
...
{
	Group* groupRoot = m_db->getRootGroup();
	...
	m_csvExporter->exportDatabase(
		&buffer,
		m_db
	);
	QString expectedResult = QString().append(
		ExpectedHeaderLine
	).append(
		"\"Test Group Name\",\"Test Entry Title\",\"Test Username\",\"Test Password\",\"http://test.url\",\"Test Notes\"\n"
	);
	QCOMPARE(
...
void TestCsvExporter::testNestedGroups()
{
	...
	QCOMPARE(
		QString::fromUtf8(buffer.buffer().constData()),
		QString().append(ExpectedHeaderLine).append(
			"\"Test Group Name/Test Sub Group Name\",\"Test Entry Title\",\"\",\"\",\"\",\"\"\n"
		)
	);
}
